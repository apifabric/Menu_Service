...
resourcename="menuservice"  # lower case, only
resourcegroup="menuservice_rg"
...
# git clone https://github.com/apilogicserver/menuservice.git
# cd menuservice
...
az group create --name menuservice_rg --location "westus"
az appservice plan create --name myAppServicePlan --resource-group menuservice_rg --sku S1 --is-linux
az container create --resource-group menuservice_rg --name menuservice --image apilogicserver/menuservice:latest --dns-name-label menuservice --ports 5656 --environment-variables 'VERBOSE'='True'  'APILOGICPROJECT_CLIENT_URI'='//menuservice.westus.azurecontainer.io:5656'
